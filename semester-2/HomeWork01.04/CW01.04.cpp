...
using namespace std;
//В тектстовом файле построчно хранится информация в виде "слово целое число". Нужно на основании файла получить бинарный файл
//из записей, где каждая запись представляется объектом класса из двух полей:строка и целое число
...
void transfer(const char* destinationPath, const char* sourcePath)
{
	...
	int bufferNumber = 0, sizeBuf = sizeof(WordNumber);
	WordNumber* bufferObj;

	while (!in.eof())
	{
		in.getline(buffer, sizeBuf);
		bufferObj = parse(buffer);
		out.write((char*)bufferObj, sizeBuf);
		delete bufferObj;
	}
...
void read(const char* source)
{
	...
	int sizeBuf = sizeof(WordNumber);
	WordNumber* bufferObj = new WordNumber();

	cout << in.tellg();

	while (in.read((char*)bufferObj, sizeBuf))
	{
