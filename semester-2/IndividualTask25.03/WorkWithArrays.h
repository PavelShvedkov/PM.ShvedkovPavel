...
namespace myArrays
{
	...
	void runTests();
	void maxElementSortMatrix(int**, int, int);
	int maxElementPosition(int*, int);
	...
	int* createKeys(int** matrix, int rows, int colmns)
	{
		int* keys = new int[rows];

		for (int i = 0; i < rows; i++)
		{
			keys[i] = matrix[i][maxElementPosition(matrix[i], colmns)];
		}

		return keys;
	}

	int maxElementPosition(int* array, int size)
	{
		int max = 0;
	...
	void maxElementSortMatrix(int** matrix, int rows, int columns)
	{
		...
		for (int i = 0; i < rows - 1; i++)
		{
			for (int j = i; j >= 0; j--)
			{
				index1 = maxElementPosition(matrix[j], columns);
				index2 = maxElementPosition(matrix[j + 1], columns);
